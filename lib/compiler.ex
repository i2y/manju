defmodule Compiler do
  ...
  def put_func(manju_syntax, func_map) do
    func_syntax = to_erl_syntax(manju_syntax)
  ...
  def put_patterns(patterns_manju_ast) do
    [:patterns, patterns_name, patterns] = patterns_manju_ast
    :erlang.put(patterns_name, patterns)
  end

  def to_erl_form_list(syntax_list, module_name) do
    module_decl = attribute(atom(:module), [atom(module_name)])
    Enum.map([module_decl|syntax_list], &revert(&1)) |> :compile.forms()
  end

  def to_erl_syntax_list([manju_syntax | manju_syntax_list], func_map, acc) do
    case manju_syntax do
      [:decorated_func, attr, func_def] ->
        to_erl_syntax_list([func_def|manju_syntax_list],
                            func_map,
      ...
      _ -> to_erl_syntax_list(manju_syntax_list, func_map, acc ++ [to_erl_syntax(manju_syntax)])
    end
  end
  ...
  def to_erl_syntax([{:call_method, line}, obj, {:name, _, method_name}, args]) do
    operator = module_qualifier(atom(:manju_runtime), atom(:call_method))
  ...
  def to_erl_syntax([{:new, line}, {:name, _, module_name}, args]) do
    operator = module_qualifier(atom(:manju_runtime), atom(:new_object))
    compiled_args = Enum.map(args, &Compiler.to_erl_syntax(&1))
  ...
  def to_erl_syntax([:get_maps_value, map_obj, {:name, line, name}]) do
    operator = module_qualifier(atom(:maps), atom(:get))
    application(operator,
                [atom(name),
                 to_erl_syntax(map_obj)])
    |> set_pos(line)
  end
